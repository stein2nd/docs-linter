...
* **柔軟な導入方法**:
  * npm パッケージとして利用可能（推奨）。Git Submodule は [Git Submodule での利用](./docs/archive/git_submodule.md) を参照（後方互換・非推奨）。
* **CI/CD 対応**:
...
* Node.js v20以上
* npm v10以上 (npm **12** 以降は Git 依存の追加設定が必要 — 下記「npm 12 と Git 依存」を参照)
* Git
...
**利用側プロジェクトに `@s2j/docs-linter` を追加する場合** … 次のいずれかが必要です。

```zsh
npm install --save-dev @s2j/docs-linter --allow-git=root
```

または、利用側プロジェクトの `.npmrc` に次を追加します。

```
allow-git=root
```

`allow-git=root` は、プロジェクトの `package.json` に直接書かれた Git 依存のみ許可します (推移的な Git 依存は取得しません)。詳細は [npm の `allow-git` 設定](https://docs.npmjs.com/cli/v12/using-npm/config#allow-git) を参照してください。

`package.json` の `engines.npm` は **npm 12** を含みます (除外対象は不具合のある **v11.12.0** のみ)。`EBADENGINE` 警告が出る場合は npm のバージョンを確認してください。

### 「npm パッケージ」の導入

npm レジストリからは **`@s2j/docs-linter`** をインストールし、CLI **`s2j-docs-linter`** で lint します (互換のため **`docs-lint`** も利用可能)。手順の詳細は [npm 使い方ガイド](docs/cli/npm_usage.md) を参照してください。
...
`docs-linter` は下記順序で、設定ファイルを検出します。
...
Xcode では直接 textlint を統合できませんが、下記方法で利用できます。
...
互換 CLI `docs-lint` でも同じ引数を指定できます。詳細は [npm 使い方ガイド](docs/cli/npm_usage.md) を参照してください。
...
GitHub Actions での CI 実行時には、下記ベスト・プラクティスを推奨します。
...
手順の詳細は [docs/archive/git_submodule.md](./docs/archive/git_submodule.md) を参照してください。
...
本プロジェクトは **textlint 用ルールセット** を提供することを目的としています。開発にあたっては下記ガイドラインに従ってください。
...
3. **lockfile を再生成する**  
   ...
   * 本リポジトリは `github:...` 形式の Git 依存を含む。リポジトリ直下の [`.npmrc`](.npmrc) (`allow-git=root`) により、**npm 12** でも通常の `npm install` が使える。`EALLOWGIT` が出る場合は [npm 12 と Git 依存](#npm-12-と-git-依存) を参照。

**npm 11.12.0 のみ** … Git 依存の準備時に `--prefer-offline` / `--prefer-online` の不整合により `npm install` が失敗することがある (たとえば `git dep preparation failed` / `--prefer-online cannot be provided when using --prefer-offline`)。その場合は **グローバルの npm を下げずに**、次のスクリプトでインストールする。

```zsh
npm run install:compat
```

`install:compat` は、一時的に **npm v11.11.0** で `install` だけを実行する。**npm v11.12.0** 単体に起因する不具合は [npm/cli#9133](https://github.com/npm/cli/issues/9133) で追跡・クローズ済みで、**v11.12.1 以降** では不要。
