...
本ページは、[npm パッケージ仕様](../../specifications/npm_package_spec.md) にもとづく実装修正の進捗を一覧化します。仕様書の [フェーズ1実装状況](../../specifications/npm_package_spec.md#フェーズ1実装状況-2026-05) と相互参照します。publish 認証・シークレット方針は [npm 認証およびシークレット管理仕様](../../specifications/npm_auth_secret_manage_spec.md) を参照してください。

**移行フェーズ** は [移行戦略 - 非推奨化ポリシー](../../specifications/npm_package_spec.md#移行戦略---非推奨化ポリシー) に従い、**フェーズ1 完了**、**フェーズ2 完了**、**フェーズ3 完了**、**フェーズ4 完了** (必須 + 推奨 **100%**)。

* フェーズ1–4 **クローズ** (必須 + 推奨)。**P2 (CLI ツール) 500/500 クローズ** (2026-06-02)。
* npm レジストリ最新 … **`@s2j/docs-linter@1.0.15`** (`v1.0.14`–`v1.0.15` GHA OIDC publish、2026-06-02)。
* **README ドキュメント構成** — npm 主導線は [README.md](../../../README.md)。Git Submodule 手順は [docs/archive/git_submodule.md](../git_submodule.md) に切り出し (後方互換・非推奨)。
...
* フェーズ4 **P2 (CLI ツール)** — [cli_tooling_spec.md](../../specifications/cli_tooling_spec.md) **100% クローズ** (2026-06-02)
    * **`init` 済** (2026-05-30) — `src/bin/init.ts` · `npm run test:init` (INIT-001〜012) · GHA `Test init command`
    * **`doctor` 済** (2026-06-02 · Fixes [#31](https://github.com/stein2nd/docs-linter/issues/31)) — `src/bin/doctor.ts` · `npm run test:doctor` (DOCTOR-001〜015) · GHA `Test doctor command`
    * **ドキュメント同期 済** (2026-06-02) — [README.md](../../../README.md) · [npm_usage.md](../../cli/npm_usage.md) · [docs/archive/git_submodule.md](../git_submodule.md) への `init` / `doctor` / Submodule 導線
    * **ローカル検証 済** (2026-06-02) — `npm run test:init` · `npm run test:doctor` すべて PASS
    * **P2 実装％ … 100%** (500 / 500)
...
| ドキュメント | 内容 |
| --- | --- |
| [npm パッケージ仕様](../../specifications/npm_package_spec.md) | npm 配布、CLI、互換、移行、GHA publish、**パッケージ構成 (artifact)** の全体仕様 |
| [npm 認証およびシークレット管理仕様](../../specifications/npm_auth_secret_manage_spec.md) | Trusted Publishing (OIDC) 登録済、`NPM_TOKEN` 非使用方針 |
...
| [npm 使い方ガイド](../../cli/npm_usage.md) | install / CLI / `init` / `doctor` / `package.json` `lint:docs` 移行 / VSCode・`extends` / CI |
| [docs/archive/git_submodule.md](../git_submodule.md) | Git Submodule 利用手順 (後方互換・非推奨)。README から導線 |
| [CLI ツール仕様](../../specifications/cli_tooling_spec.md) | `init` / `doctor` コマンド、`init` テスト仕様 (INIT-001〜012)、`doctor` テスト仕様 (DOCTOR-001〜015)、CLI エントリポイント方針 |
...
| [`.github/workflows/ci.yml`](../../../.github/workflows/ci.yml) | Node 20/22/24 matrix、`pack:check` / `verify:tarball` / `test:init` / `test:doctor` (push/PR) — 必須 #4 **済** / P2 **済** |
...
| [仕様書の起点](../../specs.md) | 仕様導線 + [プロジェクトのロードマップ](../../specs.md#ロードマップ) → [status.md](./status.md) — 推奨 #5 **済** |
| [README.md](../../../README.md) | npm 主導線 · Quick Start · [npm_usage.md](../../cli/npm_usage.md) 導線 · [git_submodule.md](../git_submodule.md) (Submodule・非推奨) · ロードマップ / ガバナンス — 推奨 #4–#5 |

## 機能一覧 (実装状況サマリー)

| 機能名 (仕様セクション) | 実装済み/未実装 | 実装％ | 完了条件 (要約) | 備考 |
| --- | --- | ---: | --- | --- |
| [Publishing - フェーズ1優先タスク](../../specifications/npm_package_spec.md#publishing---フェーズ1優先タスク) | 実装済み | 100 | pack / dry-run / tarball / registry / 受け入れ (#1–2, #4–9, #11–12) | npm 最新 **`@s2j/docs-linter@1.0.15`** (2026-06-02) |
| [Publishing (パッケージ構成) - フェーズ1優先](../../specifications/npm_package_spec.md#publishing-パッケージ構成---フェーズ1優先タスク) | 実装済み | 100 | `pack:artifact`、`artifacts/`、再現可能な検証 (#1–2, #4, #7, #16) | GHA artifact 名 `s2j-docs-linter-<tag>` |
| [npm パッケージ仕様](../../specifications/npm_package_spec.md) (文書) | 実装済み | 100 | コードと仕様の同期 | パッケージ構成セクション反映済み |
| [`package.json` メタデータ更新](../../specifications/npm_package_spec.md#packagejson-メタデータ更新) | 実装済み | 100 | `@s2j/docs-linter`、semver、`LICENSE` 等 | **`1.0.15`** (#5–6) |
| [CLI Entrypoint 公開](../../specifications/npm_package_spec.md#cli-entrypoint-公開) | 実装済み | 100 | `bin` / `--profile` / 設定解決 / `--help` / `--version` | textlint を `node` 直実行 (#3) |
| [ファイルスコープの最適化 (Publish)](../../specifications/npm_package_spec.md#ファイルスコープの最適化を-publish) | 実装済み | 100 | `files` + `verify:tarball` | `presets/` + root ミラー (#4, #9) |
| [CLI 互換レイヤ](../../specifications/npm_package_spec.md#cli-互換レイヤ) / [互換性に関する要件](../../specifications/npm_package_spec.md#互換性に関する要件) | 実装済み | 100 | パッケージ root 基準の preset 解決、互換 `docs-lint` | |
| [互換性に関する、移行戦略 - フェーズ1優先](../../specifications/npm_package_spec.md#互換性に関する移行戦略---フェーズ1優先タスク) | 実装済み | 100 | tarball に `base/` `swift/` `wordpress/` 同梱 | `npm_usage.md` に VSCode / `extends` (#13–14) |
| [依存関係レビュー](../../specifications/npm_package_spec.md#依存関係レビュー) | 実装済み | 100 | 実行時 → `dependencies`；Dependabot weekly | [`.github/dependabot.yml`](../../../.github/dependabot.yml) (必須 #3 **済**) |
| [Publishing](../../specifications/npm_package_spec.md#publishing) (全体) | 実装済み | 100 | フェーズ1 **16/16** + フェーズ2 **5/5** + フェーズ3 **9/9** 済 | フェーズ3 **クローズ** (2026-05-25) |
| [本リポジトリ `package.json` の `scripts` - フェーズ1優先](../../specifications/npm_package_spec.md#本リポジトリ-packagejson-の-scripts---フェーズ1優先タスク) | 実装済み | 100 | `clean` / `build` / `prepare` / publish 検証群 / CLI 経由 `lint*` | `postinstall` はフェーズ1非対象で現状維持 (#8) |
| [移行のワークフロー例](../../specifications/npm_package_spec.md#移行のワークフロー例) | 実装済み | 100 | `examples/lint-docs*.yml` で `npx s2j-docs-linter` | フェーズ3 #5 **済**。legacy Submodule ステップは reference |
| [移行のワークフロー例 - フェーズ1優先](../../specifications/npm_package_spec.md#移行のワークフロー例---フェーズ1優先タスク) | 実装済み | 100 | migration examples + `lint:docs` before/after | [npm_usage.md](../../cli/npm_usage.md) + `examples/` (#10, #15) |
| [互換性に関する、移行戦略](../../specifications/npm_package_spec.md#互換性に関する移行戦略) (フェーズ1) | 実装済み | 100 | 併存基盤 + root 互換レイアウト + 移行ドキュメント | **9 リポジトリ**で Submodule → npm 移行完了 (#12) |
| [README 移行](../../specifications/npm_package_spec.md#readme-移行) | **済** | 100 | npm 主導線 + [git_submodule.md](../git_submodule.md) (Submodule 非推奨) + VSCode 定着 | フェーズ3 推奨 #2–3 **済** (#6 **済**) |
| [GitHub Actions Publish ワークフロー](../../specifications/npm_package_spec.md#github-actions-publish-ワークフロー) | 運用中 | 100 | OIDC tag publish + artifact | run [`26381626088`](https://github.com/stein2nd/docs-linter/actions/runs/26381626088) |
| [GitHub Actions Publish - フェーズ1優先](../../specifications/npm_package_spec.md#github-actions-publish-ワークフロー---フェーズ1優先タスク) | 運用中 | 100 | tag `v*` / `npm ci` / 検証 / artifact / publish | OIDC 運用 (2026-05-25) |
| [npm 認証およびシークレット管理仕様](../../specifications/npm_auth_secret_manage_spec.md) | 実装済み | 100 | Trusted Publishing 登録 + ワークフロー OIDC | `NPM_TOKEN` Secret 不使用 |
| [npm 使い方ガイド との整合 - フェーズ1優先](../../specifications/npm_package_spec.md#npm-使い方ガイド-との整合---フェーズ1優先タスク) / [npm_usage.md](../../cli/npm_usage.md) | 実装済み | 100 | README のコマンドが実際に動く · `init` / `doctor` 導線 | `lint:docs` / VSCode / `extends` / CI (#10, #14) · P2 **済** |
| P2 ドキュメント同期 (CLI) | **済** | 100 | README / [npm_usage.md](../../cli/npm_usage.md) / [git_submodule.md](../git_submodule.md) への `init` / `doctor` / Submodule 導線 | 更新 **2026-06-02** · 仕様 [ドキュメントの同期](../../specifications/cli_tooling_spec.md#ドキュメントの同期) |
| [VSCode 互換戦略](../../specifications/npm_package_spec.md#vscode-互換戦略) | ガイド済み | 100 | [npm_usage.md](../../cli/npm_usage.md) に移行例 | 利用側 **9 リポ** canonical 設定 (フェーズ3 推奨 #3 **済**) |
| [CI 互換戦略](../../specifications/npm_package_spec.md#ci-互換戦略) | ガイド済み | 100 | `npm_usage.md` / `examples/` に CI 例 | 利用側 **9 リポ** GHA docs lint 成功 (フェーズ3 #5 **済**) |
| Node matrix CI (フェーズ4) | **済** | 100 | Node 20/22/24 互換検証 · `test:init` · `test:doctor` | [ci.yml](../../../.github/workflows/ci.yml) (必須 #4 **済**) |
| [バージョン管理ポリシー](../../specifications/npm_package_spec.md#バージョン管理ポリシー) | **済** | 100 | semver 運用 + CHANGELOG | [versioning_policy.md](../../versioning_policy.md) (必須 #1 **済**) |
...
| [移行戦略 - 非推奨化ポリシー](../../specifications/npm_package_spec.md#移行戦略---非推奨化ポリシー) フェーズ3 | **済** | 100 | Submodule 非推奨化・利用側 cutover | 必須 #1–6 + 推奨 #1–3 **済** (2026-05-25) |
| [移行戦略 - 非推奨化ポリシー](../../specifications/npm_package_spec.md#移行戦略---非推奨化ポリシー) フェーズ4 | **クローズ** | 100 | レガシー機能削除・OSS 成熟化 | 必須 **5/5 済** + 推奨 **5/5 済** (2026-05-26) |
| [CLI コマンド - init](../../cli/init.md#cli-コマンド---init) | **済** | 100 | 通常 / Dry Run / `--output` モード、プリセット、上書き、`package.json` `lint:docs` | main **2026-05-30** · npm **`1.0.14`–`1.0.15`** |
| [CLI コマンド - init テスト仕様](../../cli/init.md#cli-コマンド---init-テスト仕様) | **済** | 100 | INIT-001〜012 · `scripts/test-init.sh` · GHA 再現 | 検証 **2026-06-02** PASS |
| [CLI コマンド - doctor](../../cli/doctor.md#cli-コマンド---doctor-コマンド) | **済** | 100 | 環境診断 (PASS / WARN / FAIL)、7 項目チェック、`--path` / `--help` | main **2026-06-02** · npm **`1.0.14`–`1.0.15`** · Fixes [#31](https://github.com/stein2nd/docs-linter/issues/31) |
| [CLI コマンド - doctor テスト仕様](../../cli/doctor.md#cli-コマンド---doctor-テスト仕様) | **済** | 100 | DOCTOR-001〜015 · フィクスチャ · GHA 再現 | 検証 **2026-06-02** PASS |

## フェーズ1: サマリー

仕様のフェーズ1優先タスク ([Publishing](../../specifications/npm_package_spec.md#publishing---フェーズ1優先タスク)、[**Publishing (パッケージ構成)**](../../specifications/npm_package_spec.md#publishing-パッケージ構成---フェーズ1優先タスク)、[本リポジトリ `scripts`](../../specifications/npm_package_spec.md#本リポジトリ-packagejson-の-scripts---フェーズ1優先タスク)、[移行のワークフロー例](../../specifications/npm_package_spec.md#移行のワークフロー例---フェーズ1優先タスク)、[互換移行戦略](../../specifications/npm_package_spec.md#互換性に関する移行戦略---フェーズ1優先タスク)、[npm 使い方ガイド整合](../../specifications/npm_package_spec.md#npm-使い方ガイド-との整合---フェーズ1優先タスク)、[GitHub Actions Publish](../../specifications/npm_package_spec.md#github-actions-publish-ワークフロー---フェーズ1優先タスク)) と本ページの完了条件を一致させています。

| 項目 | 状態 |
| --- | --- |
| 移行フェーズ | **フェーズ1 完了** **16/16 済**。**フェーズ2 完了** **5/5 済**。**フェーズ3 完了** **必須 6/6 + 推奨 3/3 済**。**フェーズ4 完了** 必須 **5/5** + 推奨 **5/5 済** (2026-05-26)。**P2 (CLI) 500/500 済** (2026-06-02) |
...
| --- | ---: | --- | ---: |
| [Publishing](../../specifications/npm_package_spec.md#publishing---フェーズ1優先タスク) | #1–2, #4–9, #11–12 | 済 | 100 |
| [Publishing (パッケージ構成)](../../specifications/npm_package_spec.md#publishing-パッケージ構成---フェーズ1優先タスク) | #1–2, #3–4, #7, #16 | 済 | 100 |
| [本リポジトリ `scripts`](../../specifications/npm_package_spec.md#本リポジトリ-packagejson-の-scripts---フェーズ1優先タスク) | #7–8 | 済 | 100 |
| [移行のワークフロー例](../../specifications/npm_package_spec.md#移行のワークフロー例---フェーズ1優先タスク) | #10, #15 | 済 | 100 |
| [互換性・移行戦略](../../specifications/npm_package_spec.md#互換性に関する移行戦略---フェーズ1優先タスク) | #13–14 | 済 | 100 |
| [npm 使い方ガイド整合](../../specifications/npm_package_spec.md#npm-使い方ガイド-との整合---フェーズ1優先タスク) | #3, #10 | 済 | 100 |
| [GitHub Actions Publish](../../specifications/npm_package_spec.md#github-actions-publish-ワークフロー---フェーズ1優先タスク) | #16 (artifact 保持) | 済 (運用) | 100 |
| [npm 認証・シークレット管理](../../specifications/npm_auth_secret_manage_spec.md) (文書) | — | 済 (方針) | 100 |
...
| 7 | publish 用 `scripts` (`pack:check`, `pack:artifact`, `publish:dry-run`, `lint:package`, `verify:tarball`, `verify:artifact`) | **済** | 100 | [仕様の標準 scripts](../../specifications/npm_package_spec.md#publishing-パッケージ構成---フェーズ1優先タスク) + `verify:*` |
...
| 10 | README / [npm_usage.md](../../cli/npm_usage.md) / install examples の整合 | **済** | 100 | npm 主導線 ([README](../../../README.md)) · Submodule ([git_submodule.md](../git_submodule.md)) · [npm_usage.md](../../cli/npm_usage.md) |
...
| 14 | VSCode / `extends` の移行ガイド ([npm_usage.md](../../cli/npm_usage.md)) | **済** | 100 | Submodule → `node_modules/@s2j/docs-linter/{swift,presets/*,base}/` の before/after |
| 15 | 移行のワークフロー例 (`examples/` + `lint:docs` 移行ドキュメント) | **済** | 100 | [examples/lint-docs*.yml](../../../examples/lint-docs.yml) の `npx s2j-docs-linter`；[npm_usage.md](../../cli/npm_usage.md) の Submodule → npm 表 |
...
仕様 [npm 配布のための実装修正](../../specifications/npm_package_spec.md#npm-配布のための実装修正) の責務表に対応します。
...
| README の更新 (フェーズ1範囲) | 済 | 100 | フェーズ1時点: Submodule 主導線 + npm 併記。現在: npm 主導線 + [git_submodule.md](../git_submodule.md) (#10) |
| [npm 使い方ガイド](../../cli/npm_usage.md) との整合 | 済 | 100 | install / CLI / `lint:docs` / CI / VSCode・`extends` が README・仕様と一致 (#10, #14) |
...
| npm 認証・シークレット管理 (文書) | 済 | 100 | [npm_auth_secret_manage_spec.md](../../specifications/npm_auth_secret_manage_spec.md) — Trusted Publishing 登録済 |

## フェーズ1で完了した項目

仕様 [フェーズ1で完了した項目](../../specifications/npm_package_spec.md#フェーズ1で完了した項目) および各フェーズ1優先タスクの責務と一致します。
...
* README への npm 手順の併記 (フェーズ1時点は Submodule 主導線) および [npm_usage.md](../../cli/npm_usage.md) への導線 — 現在は npm 主導線、Submodule 手順は [docs/archive/git_submodule.md](../git_submodule.md)
* [npm_usage.md](../../cli/npm_usage.md) … install / CLI / **`package.json` `lint:docs` 移行例 (WordPress・Swift)** / GitHub Actions / **VSCode・`extends` 移行例**
...
* [npm_auth_secret_manage_spec.md](../../specifications/npm_auth_secret_manage_spec.md) … Trusted Publishing (OIDC) 登録済、`NPM_TOKEN` 非使用
...
| `README.md` | npm 主導線を `@s2j/docs-linter` / `s2j-docs-linter` に更新、Submodule は [git_submodule.md](../git_submodule.md)、`npm_usage.md` へ導線 | #10 |
| `docs/cli/npm_usage.md` | `lint:docs` before/after、`--profile`、CI、VSCode・`extends` 移行例 | #10, #14, #15 |
| `docs/specifications/npm_package_spec.md` | フェーズ1優先タスク・**パッケージ構成**・GHA・残タスクの注記 | 文書 |
| `docs/specifications/npm_auth_secret_manage_spec.md` | 認証優先順位 (OIDC > 自動化トークン > 手動)、secret 運用、GHA 例 | 文書 |
| `docs/specs.md` | 認証仕様・実装状況への導線 | 文書 |
...
| 3 | publish secret / auth strategy が仕様化済 | **済** ([npm_auth_secret_manage_spec.md](../../specifications/npm_auth_secret_manage_spec.md)) | 100 |
...
| `docs/release.md` | Trusted Publishing 手順、ローカル publish トラブルシュート | 推奨 #2, O1 |
...
| npm レジストリ (公開済) | **済** — 最新 **`@s2j/docs-linter@1.0.15`** (2026-06-02) |
| README トップ導線 (UX) | **済** — npm 主導線。Submodule は [git_submodule.md](../git_submodule.md) (非推奨) |
...
| P2 | マイグレーションドキュメントの完成 | README / docs 更新完了 | **済** ([npm_usage.md](../../cli/npm_usage.md) + [README](../../../README.md) legacy セクション) | 100 |
...
| 2 | マイグレーションガイドの完成 | **済** | 100 | [npm_usage.md](../../cli/npm_usage.md) に migration guide 記載 (`lint:docs` / VSCode / GitHub Actions)。利用側 **9 リポ** で migration 実施済。Git Submodule → npm 置換実績あり |
| 3 | VSCode 設定例の完成 | **済** | 100 | [npm_usage.md](../../cli/npm_usage.md) に VSCode migration before/after。利用側 **9 リポ** で npm package ベース設定へ移行済。canonical: `.vscode/settings.json` — `textlint.configPath=./.textlintrc.json`, `textlint.nodePath=./node_modules`, `textlint.autoFixOnSave`, `source.fixAll.textlint`。`.textlintrc.json` — `@s2j/docs-linter` preset `extends` |

**備考 (推奨 #2–3)**: README は npm 主導線。Git Submodule 手順は [docs/archive/git_submodule.md](../git_submodule.md) に切り出し (非推奨)。VSCode 設定は実利用ベースで標準設定として定着。
...
* **推奨 #2**: [npm_usage.md](../../cli/npm_usage.md) migration guide 完成 — 利用側 **9 リポ** 実施済
...
| [README.md](../../../README.md) | npm 主導線 · 「レガシー移行リファレンス」— Submodule 非推奨 · 詳細は [git_submodule.md](../git_submodule.md) | 必須 #6 |
| [docs/archive/git_submodule.md](../git_submodule.md) | Git Submodule 利用手順 (後方互換・非推奨) — README から切り出し | ドキュメント構成 **2026-06-02** |
| [examples/lint-docs*.yml](../../../examples/lint-docs.yml) | npm workflow 手順 + legacy Submodule reference 併記 | 必須 #5 |
| [npm_usage.md](../../cli/npm_usage.md) | `lint:docs` / VSCode / CI 移行ガイド | 推奨 #2–3 |
...
| **Node matrix CI** | **済** — [ci.yml](../../../.github/workflows/ci.yml) Node 20/22/24 · `test:init` · `test:doctor` (必須 #4) |
...
| **ロードマップ** | **済** — 本ページ (フェーズ1–4 進捗) + [specs.md](../../specs.md#ロードマップ) + [README.md](../../../README.md#ロードマップ) (推奨 #5) |
...
| **CLI `init` コマンド (P2)** | **済** — main **2026-05-30** · [src/bin/init.ts](../../../src/bin/init.ts) · `npm run test:init` (INIT-001〜012) · GHA [ci.yml](../../../.github/workflows/ci.yml) |
| **CLI `doctor` コマンド (P2)** | **済** — main **2026-06-02** (Fixes [#31](https://github.com/stein2nd/docs-linter/issues/31)) · [src/bin/doctor.ts](../../../src/bin/doctor.ts) · `npm run test:doctor` (DOCTOR-001〜015) · GHA [ci.yml](../../../.github/workflows/ci.yml) |
| **P2 CLI 実装 (コード・テスト・GHA)** | **100%** — init / doctor 本実装 + 自動テスト + CI 統合 **済** (検証 **2026-06-02**) |
| **P2 ドキュメント同期** | **100%** — [README.md](../../../README.md) · [npm_usage.md](../../cli/npm_usage.md) · [git_submodule.md](../git_submodule.md) への `init` / `doctor` / Submodule 導線 **済** (更新 **2026-06-02**) |
| **P2 (CLI ツール) 全体** | **100%** (500 / 500) · **クローズ 2026-06-02** |
| **npm 公開 (P2 機能同梱)** | **済** — **`@s2j/docs-linter@1.0.14`–`1.0.15`** (2026-06-02) |
...
**P2 (CLI ツール) クローズ**: **達成** (2026-06-02) — `init` (2026-05-30) + `doctor` (2026-06-02) + ドキュメント同期 + 自動テスト **500/500**。
...
**P2 内訳 (CLI ツール)** — 仕様 [cli_tooling_spec.md](../../specifications/cli_tooling_spec.md)

| コマンド (仕様見出し) | 完了条件 (要約) | 状態 | 実装％ |
| --- | --- | --- | ---: |
| [CLI コマンド - init](../../cli/init.md#cli-コマンド---init) | 実行モード (通常 / Dry Run / `--output`) · 引数 (`--preset` / `--force` 等) · 3 ファイル + `lint:docs` 生成 · 上書きポリシー · サブコマンドルーター (`cli.js`) | **済** | 100 |
| [CLI コマンド - init テスト仕様](../../cli/init.md#cli-コマンド---init-テスト仕様) | INIT-001〜012 · `scripts/test-init.sh` · GHA 再現 · `.sandbox/` 検証 | **済** | 100 |
| [CLI コマンド - doctor](../../cli/doctor.md#cli-コマンド---doctor-コマンド) | 7 項目診断 (`package.json` / Node.js / Config / Preset / textlint / VSCode / GitHub Actions) · PASS / WARN / FAIL 出力 · `--path` / `--help` | **済** | 100 |
| [CLI コマンド - doctor テスト仕様](../../cli/doctor.md#cli-コマンド---doctor-テスト仕様) | DOCTOR-001〜015 · `scripts/test-doctor.sh` · フィクスチャ · GHA 再現 | **済** | 100 |
| P2 ドキュメント同期 | README / [npm_usage.md](../../cli/npm_usage.md) への `init` / `doctor` 導線 | **済** | 100 | 更新 **2026-06-02** |
...
| # | 完了条件 | 状態 | 実装％ | 検証方法 |
| ---: | --- | --- | ---: | --- |
| I1 | 仕様書 ([init.md](../../cli/init.md#cli-コマンド---init)) | **済** | 100 | 実行モード · 引数 · 生成ファイル · 上書きポリシー |
...
| I6 | CLI エントリポイント (`bin` → `cli.js`) | **済** | 100 | `init` / `doctor` / lint (デフォルト) ルーティング |
| I7 | 自動テスト (INIT-001〜012) | **済** | 100 | `npm run test:init` · [scripts/test-init.sh](../../../scripts/test-init.sh) — **2026-06-02** PASS |
...
| # | 完了条件 | 状態 | 実装％ | 検証方法 |
| ---: | --- | --- | ---: | --- |
| D1 | 仕様書 ([doctor.md](../../cli/doctor.md#cli-コマンド---doctor-コマンド)) | **済** | 100 | 診断範囲 · PASS / WARN / FAIL 出力ポリシー |
| D2 | `doctor` 本実装 (7 項目チェック) | **済** | 100 | `package.json` / Node.js / Config / Preset / textlint / VSCode / GitHub Actions |
| D3 | `doctor` テスト仕様 (DOCTOR-001〜015) | **済** | 100 | [doctor.md](../../cli/doctor.md#cli-コマンド---doctor-テスト仕様) · フィクスチャ定義 |
| D4 | 自動テスト | **済** | 100 | `npm run test:doctor` · [scripts/test-doctor.sh](../../../scripts/test-doctor.sh) — **2026-06-02** PASS |
| D5 | ルーター統合 | **済** | 100 | [cli.ts](../../../src/bin/cli.ts) → [doctor.ts](../../../src/bin/doctor.ts) |
| D6 | GitHub Actions 統合 | **済** | 100 | [ci.yml](../../../.github/workflows/ci.yml) `Test doctor command` ステップ |
...
| # | 完了条件 | 状態 | 実装％ | 検証方法 |
| ---: | --- | --- | ---: | --- |
| S1 | [README.md](../../../README.md) — `init` 導線 (セットアップ) | **済** | 100 | `npx @s2j/docs-linter init` · 生成ファイル一覧 — 更新 **2026-06-02** |
| S2 | [README.md](../../../README.md) — `doctor` 導線 (環境チェック) | **済** | 100 | `npx @s2j/docs-linter doctor` · `--path` · 出力例 — 更新 **2026-06-02** |
| S3 | [npm_usage.md](../../cli/npm_usage.md) — 初回セットアップ (`init`) | **済** | 100 | `npx @s2j/docs-linter init` · 生成ファイル — 更新 **2026-06-02** |
| S4 | [npm_usage.md](../../cli/npm_usage.md) — 設定確認 (`doctor`) | **済** | 100 | `doctor` コマンド · 診断項目 · CI 導入後の確認例 — 更新 **2026-06-02** |
...
| 4 | Node バージョンの互換性 CI | **済** | 100 | [`.github/workflows/ci.yml`](../../../.github/workflows/ci.yml) — matrix Node 20/22/24、`pack:check` / `verify:tarball` / `test:init` / `test:doctor`。publish は [npm-publish.yml](../../../.github/workflows/npm-publish.yml) Node 24 |
...
| 5 | ロードマップの可視化 | **済** | 100 | 本ページ ([status.md](./status.md)) — フェーズ1–4 進捗・完了条件。[specs.md](../../specs.md#ロードマップ) — ロードマップ導線・フェーズ概要。[README.md](../../../README.md#ロードマップ) — status / ガバナンス ([maintenance_policy.md](../../maintenance_policy.md)、[versioning_policy.md](../../versioning_policy.md)) 導線 |
...
* **必須 #4**: [`.github/workflows/ci.yml`](../../../.github/workflows/ci.yml) — Node 20/22/24 matrix、`pack:check` / `verify:tarball` / `test:init` / `test:doctor`
...
* **推奨 #5**: ロードマップ可視化 — 本ページ (フェーズ1–4 進捗・完了条件)、[specs.md](../../specs.md#ロードマップ) (status 導線・フェーズ概要)、[README.md](../../../README.md#ロードマップ) (status / ガバナンス導線)
* **P2 / CLI `init`**: main **2026-05-30** — [init.md](../../cli/init.md) 仕様 · [src/bin/init.ts](../../../src/bin/init.ts) · [scripts/test-init.sh](../../../scripts/test-init.sh) · `npm run test:init` (INIT-001〜012) · [ci.yml](../../../.github/workflows/ci.yml) GHA 統合
* **P2 / CLI `doctor`**: main **2026-06-02** (Fixes [#31](https://github.com/stein2nd/docs-linter/issues/31)) — [doctor.md](../../cli/doctor.md) 仕様 · [src/bin/doctor.ts](../../../src/bin/doctor.ts) · [scripts/test-doctor.sh](../../../scripts/test-doctor.sh) · `npm run test:doctor` (DOCTOR-001〜015) · [ci.yml](../../../.github/workflows/ci.yml) GHA 統合
* **P2 / ドキュメント同期**: **2026-06-02** — [README.md](../../../README.md) · [npm_usage.md](../../cli/npm_usage.md) · [docs/archive/git_submodule.md](../git_submodule.md) — `init` / `doctor` / Submodule 導線
...
| [`.github/workflows/ci.yml`](../../../.github/workflows/ci.yml) | Node 20/22/24 matrix CI · `test:init` · `test:doctor` | 必須 #4 / P2 |
...
| [README.md](../../../README.md) | `init` (セットアップ) · `doctor` (環境チェック) · [npm_usage.md](../../cli/npm_usage.md) 導線 | P2 / S1–S2 · **2026-06-02** |
| [docs/archive/git_submodule.md](../git_submodule.md) | Git Submodule 利用手順 (非推奨) — README から切り出し | ドキュメント構成 · **2026-06-02** |
| [npm_usage.md](../../cli/npm_usage.md) | 初回セットアップ (`init`) · 設定確認 (`doctor`) · 診断項目 · CI 導入後の確認例 | P2 / S3–S4 · **2026-06-02** |
...
| [cli_tooling_spec.md](../../specifications/cli_tooling_spec.md) | CLI 共通方針 | P2 |
| [doctor.md](../../cli/doctor.md) | `doctor` / doctor テスト仕様 (DOCTOR-001〜015) | P2 |
| [init.md](../../cli/init.md) | `init` / init テスト仕様 (INIT-001〜012) | P2 |
| [src/bin/cli.ts](../../../src/bin/cli.ts) | サブコマンドルーター (`init` / `doctor` / lint) · `bin` → `dist/bin/cli.js` | P2 / I6 / D5 |
| [src/bin/doctor.ts](../../../src/bin/doctor.ts) | `doctor` — 7 項目診断 · PASS / WARN / FAIL · `--path` / `--help` | P2 / D2 |
...
| `package.json` | `test:init` · `test:doctor` script · `bin` / `main` → `dist/bin/cli.js` | P2 |

## フェーズ4の残タスク

**なし** — フェーズ4必須条件 #1–5、推奨条件 #1–5、**P2 (CLI ツール) 500/500**、および P2 機能の npm 公開 (**`1.0.14`–`1.0.15`**, 2026-06-02) はすべて **済**。

## 補足

* **公開 API**:
    * `presets/{base,swift,wordpress}/` 配下パス、root 互換 `base/` `swift/` `wordpress/`、CLI 名、`extends` 用相対パスは互換契約 ([現在の利用状況の分析](../../specifications/npm_package_spec.md#現在の利用状況の分析))。
* **ビルド**:
...
* **CLI ツール (P2)**:
    ...
    * ユーザー向け導線 … [README.md](../../../README.md) · [npm_usage.md](../../cli/npm_usage.md) · [docs/archive/git_submodule.md](../git_submodule.md) に `init` / `doctor` / Submodule 手順 **済**。
* **root ミラー**:
...
* **GHA と認証**:
    * **CI matrix** ([ci.yml](../../../.github/workflows/ci.yml)): `push` / `pull_request`、Node 20/22/24、`npm ci` → `pack:check` → `verify:tarball` → `test:init` → `test:doctor` (フェーズ4 必須 #4 **済** / P2 **init・doctor 済**)。
    ...
    * **ロードマップ** ([status.md](./status.md) · [specs.md](../../specs.md#ロードマップ) · [README.md](../../../README.md#ロードマップ)): フェーズ1–4 進捗・完了条件、フェーズ概要、ガバナンス ([maintenance_policy.md](../../maintenance_policy.md) / [versioning_policy.md](../../versioning_policy.md)) 導線 — 推奨 #5 **済**。
...
* **npm publish / 受け入れ試験**:
    * レジストリ最新 … **`@s2j/docs-linter@1.0.15`** (GHA OIDC、2026-06-02)
    * **`init` / `doctor`** … npm **`1.0.14`–`1.0.15`** に同梱済
    * GHA OIDC publish … **`1.0.13`** 初回 · 以降 **`1.0.14`–`1.0.15`** / 手動 publish … **`1.0.10`–`1.0.12`**
    * 受け入れ (#12) / フェーズ3 #1–2 … **9 リポジトリ** npm 化・`tools/docs-linter` 削除済
    * フェーズ3 #5 … GHA docs lint **済** (npm workflow + 利用側 **9 リポ** 実績)
    * フェーズ3 #6 … Submodule 依存削除 **済** ([README](../../../README.md) legacy 非推奨 + [git_submodule.md](../git_submodule.md) + 利用側 **9 リポ**)
* **フェーズ3 / 4**:
    ...
    * フェーズ4 **P2 (CLI ツール)** … **クローズ** (2026-06-02) — `init` (2026-05-30) / `doctor` (2026-06-02) 本実装 + 自動テスト + GHA + [README.md](../../../README.md) / [npm_usage.md](../../cli/npm_usage.md) / [git_submodule.md](../git_submodule.md) ドキュメント同期 **500/500**
    * P2 ローカル検証 … **2026-06-02** — `npm run test:init` · `npm run test:doctor` すべて PASS
    ...
    * フェーズ4 推奨 #5 … ロードマップ **済** — [status.md](./status.md) · [specs.md](../../specs.md#ロードマップ) · [README.md](../../../README.md#ロードマップ)
* **ローカル検証の一式** (2026-06-02 確認済み):

```zsh
npm run verify:tarball
npm run test:init      # INIT-001〜012 PASS
npm run test:doctor    # DOCTOR-001〜015 PASS
npm view @s2j/docs-linter version  # => 1.0.15
npm run lint
```
