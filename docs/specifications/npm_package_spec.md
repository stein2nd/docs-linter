...
フェーズ1では Git Submodule と npm パッケージの **併存** を目的とし、本リポジトリ側では npm 配布の基盤 (メタデータ、CLI、`files`、実行時依存) を整備済みです。進捗の一覧は [実装状況](../archive/mod-npm/status.md) を参照してください。

| 区分 | フェーズ1の状態 |
| --- | --- |
| 実装済み | `@s2j/docs-linter` メタデータ、`s2j-docs-linter` CLI、`presets/` の tarball 同梱、実行時 `dependencies` 移行、README の npm 手順 (併記)、`pack:artifact` / `artifacts/` ([パッケージ構成](./npm_package_spec.md#publishing-パッケージ構成---フェーズ1優先タスク)) |
...
## Publishing (パッケージ構成) - フェーズ1優先タスク

### 設計意図 (ゴール)

S2J Docs Linter を npm パッケージ (`@s2j/docs-linter`) として安定して publish 可能な構成とします。

ローカル開発環境および GitHub Actions パッケージ publish 前の検証を再現可能にし、パッケージ構成の妥当性を継続的に確認できる状態を実現します。

また、生成される tarball をバージョン単位で識別、保持可能とし、リリース artifact としての追跡性を確保します。

### 設計原則

* まず npm パッケージとして成立することを優先すること。
  * publish 自動化より前に、ローカル環境でパッケージ検証が成功することを必須とする。
* 同一ソース / 同一バージョン / 同一ビルド条件において、常に同一パッケージコンテンツを生成できること。
  * パッケージの再現性を担保し、CI / ローカル / リリース間で挙動差異を最小化する。
* パッケージ検証のためにリポジトリ root を不要に汚染しないこと。
  * CI では ephemeral ワークスペースを前提とし、artifact は専用ディレクトリに出力する。
* 生成された tarball は、バージョン単位で識別可能であること。
  * GitHub Actions artifact および npm レジストリ上で、バージョンごとの履歴追跡を可能とする。
* publish 自動化より先に、手動検証を成功させること。

生成 tarball は、下記のようにバージョン付きで生成されます。

```text
s2j-docs-linter-1.0.0.tgz
s2j-docs-linter-1.1.0.tgz
```

フェイズ1での手動検証は、下記を成功条件とします。
...
S2J Docs Linter のパッケージ検証 / publishing に関する規約は、下記の通りです。

* ローカルおよび CI のパッケージ検証は、`npm pack --dry-run` を標準とする。
* 実 tarball が必要な場合、生成先はリポジトリ root ではなく専用 artifact ディレクトリ `./artifacts` とする。
  * パッケージ tarball を専用 artifact ディレクトリに生成するコマンドは、`npm pack --pack-destination ./artifacts` となる。
  * パッケージは、`@s2j/docs-linter` となる。
* 正式 publish は、npmjs.com レジストリを利用する。
  * publish コマンドは、`npm publish --access public` となる。
* GitHub Actions artifact、npm レジストリ共に、セマンティックバージョニングに従い、バージョン履歴を保持する。

本仕様の標準 scripts は、下記とします。
...
npm レジストリ側でのバージョン履歴は、下記のようになります。

```text
@s2j/docs-linter@1.0.0
@s2j/docs-linter@1.1.0
@s2j/docs-linter@1.2.0
```

GitHub Actions artifact での tarball 履歴は、下記のようになります。
...
* パッケージ検証戦略
* tarball artifact 生成戦略
* npm publish 検証
* バージョン管理された artifact 保持方針
* パッケージスクリプトの標準化
...
Git サブモジュール運用から npm モジュール運用へ移行するにあたり、**本リポジトリ** (S2J Docs Linter) の `package.json` に定義された `scripts` のうち、見直しが必要になり得るものと、現状のままでよいものを次に整理します。利用側プロジェクトの `scripts` や CI の書き換えは、[npm 使い方ガイド](../cli/npm_usage.md) を参照します。
...
* **ライセンス**: `MIT` ではなく **`GPL-3.0-or-later`** とする。
...
* [npm_usage.md](../cli/npm_usage.md) — `package.json` の `lint:docs` について Submodule → npm の before/after を追記 ([移行のワークフロー例 - フェーズ1優先タスク](#移行のワークフロー例---フェーズ1優先タスク))。
...
* [npm_usage.md](../cli/npm_usage.md) の「`package.json` 統合」に、WordPress / Swift 向け `lint:docs` の before/after 表を追記。
...
* `examples/lint-docs*.yml` の `npx s2j-docs-linter` 化と [npm_usage.md](../cli/npm_usage.md) の `lint:docs` 移行例
