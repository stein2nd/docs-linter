...
Git Submodule 導線は [git_submodule.md](./git_submodule.md) を参照してください（非推奨・後方互換）。npm の手順は [README](../README.md) の Quick Start / Installation と本ガイドで揃えています。パッケージ全体の仕様は [npm パッケージ仕様](./npm_package_spec.md) を参照してください。
...
**`presets/` パスを使う場合** ([git_submodule.md](./git_submodule.md) の Submodule 構成と同じ) — こちらも引き続き利用できます。
