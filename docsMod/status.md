...
最終更新 …2026-05-17 (`@s2j/docs-linter@1.0.10`、root 互換レイアウト `base/` `swift/` `wordpress/` ミラー同梱・tarball 22 entries 検証済み・[npm_usage.md](./npm_usage.md) VSCode 移行例追記)
...
| [npm 使い方ガイド](./npm_usage.md) | 利用側プロジェクト向け手順 (VSCode / `extends` 移行例含む) |
| [仕様書の起点](./specs.md) | 上記への導線 |

### フェーズ1: サマリー

仕様書 [概要](./npm_package_spec.md#概要)、[Publishing - フェーズ1優先タスク](./npm_package_spec.md#publishing---フェーズ1優先タスク)、[本リポジトリ `package.json` の `scripts` - フェーズ1優先タスク](./npm_package_spec.md#本リポジトリ-packagejson-の-scripts---フェーズ1優先タスク)、[互換性に関する、移行戦略 - フェーズ1優先タスク](./npm_package_spec.md#互換性に関する移行戦略---フェーズ1優先タスク)、[npm 使い方ガイド との整合 - フェーズ1優先タスク](./npm_package_spec.md#npm-使い方ガイド-との整合---フェーズ1優先タスク) と一致させています。

| 項目 | 状態 |
| --- | --- |
| 移行フェーズ | **フェーズ1** 進行中 (併存) — 本リポジトリのコード・文書・検証は **完了**、レジストリ公開と受け入れ試験のみ未 |
| **本リポジトリ 実装％** | **100%** — フェーズ1で本リポジトリが担う責務 (#1–10, #13–14) はすべて完了 |
| **フェーズ1全体 実装％** | **86%** — 完了条件 14 項目中 12 項目済 (下表「フェーズ1完了条件」参照) |
...
| README / npm_usage / examples 整合 | **済** — 表記・CLI・install 手順・VSCode 移行例を一致 |
| npm レジストリ公開 | **未実施** — 手動 `npm publish --access public` 待ち |

| 区分 (仕様書) | フェーズ1の状態 |
| --- | --- |
| 実装済み | メタデータ、CLI (`--help` / `--version` 含む)、`bin` 互換 `docs-lint`、`scripts` 整理、本リポジトリ `lint*` の CLI 経由化、tarball 検証 (22 entries)、root 互換レイアウト、`examples/` の `npx s2j-docs-linter` 化、`dependencies` 移行、README npm 手順 (併記)、[npm_usage.md](./npm_usage.md) 拡充・VSCode 移行例 |
...
| スコープ | 分子 / 分母 | 実装％ |
| --- | --- | ---: |
| フェーズ1全体 | 完了条件 #1–14 のうち **済** の件数 | **12 / 14 → 86%** |
| 本リポジトリ責務のみ | #1–10, #13–14 (計 12 件) | **12 / 12 → 100%** |
| 本リポジトリ外 (フェーズ1クローズに必要) | #11–12 (計 2 件) | **0 / 2 → 0%** |

### フェーズ1完了条件

仕様 [Publishing - フェーズ1優先タスク](./npm_package_spec.md#publishing---フェーズ1優先タスク)、[本リポジトリ `package.json` の `scripts` - フェーズ1優先タスク](./npm_package_spec.md#本リポジトリ-packagejson-の-scripts---フェーズ1優先タスク)、[互換性に関する、移行戦略 - フェーズ1優先タスク](./npm_package_spec.md#互換性に関する移行戦略---フェーズ1優先タスク)、[npm 使い方ガイド との整合 - フェーズ1優先タスク](./npm_package_spec.md#npm-使い方ガイド-との整合---フェーズ1優先タスク) のゴール・責務に対応します。
...
| 4 | tarball 内容検証 (`presets/` + root 互換パス) | **済** | 100 | `npm run verify:tarball` — 22 entries、`package/{base,swift,wordpress}/` と `package/presets/*/` を検証 |
...
| 10 | README / [npm_usage.md](./npm_usage.md) / install examples の整合 | **済** | 100 | install コマンド・CLI 名・`--profile` 例；README 方法2 ↔ `npm_usage.md` |
...
| 14 | VSCode / `extends` の移行ガイド ([npm_usage.md](./npm_usage.md)) | **済** | 100 | Submodule → `node_modules/@s2j/docs-linter/{swift,presets/*}/` の before/after 例 |

**集計**: 完了 **12 / 14** (実装％ **86%**)。本リポジトリ責務 (#1–10, #13–14) は **12 / 12 (100%)**。
...
| npm publish に対応する tarball 構成 | 済 | 100 | `verify:tarball` で必須・禁止パスを自動検証 (22 entries) |
| レイアウト互換 (`base/` `swift/` `wordpress/`) | 済 | 100 | ビルド時 `presets/*` ミラー、VSCode パス差し替えのみで移行可 |
...
| [npm 使い方ガイド](./npm_usage.md) との整合 | 済 | 100 | install / CLI / CI / VSCode・`extends` 移行例が README・仕様と一致 |
...
| --- | --- | ---: | --- | --- |
| [Publishing - フェーズ1優先タスク](./npm_package_spec.md#publishing---フェーズ1優先タスク) | 一部 | 90 | ローカル pack / dry-run / tarball 検証が成功 | レジストリ初回 publish (#11) のみ未 |
...
| [ファイルスコープの最適化 (Publish)](./npm_package_spec.md#ファイルスコープの最適化を-publish) | 実装済み | 100 | `files` + `verify:tarball` | `presets/` + root ミラー、`scripts/patch-…` のみ同梱 |
...
| [互換性に関する、移行戦略 - フェーズ1優先](./npm_package_spec.md#互換性に関する移行戦略---フェーズ1優先タスク) | 実装済み | 100 | tarball に `base/` `swift/` `wordpress/` 同梱 | `npm_usage.md` に VSCode / `extends` 例 |
| [依存関係レビュー](./npm_package_spec.md#依存関係レビュー) | 実装済み | 100 | 実行時 → `dependencies` | |
| [Publishing](./npm_package_spec.md#publishing) (全体) | 一部 | 86 | 完了条件 12/14 済 | #11–12 未。自動 tag publish はフェーズ2 |
...
| [npm 使い方ガイド との整合 - フェーズ1優先](./npm_package_spec.md#npm-使い方ガイド-との整合---フェーズ1優先タスク) / [npm_usage.md](./npm_usage.md) | 実装済み | 100 | README のコマンドが実際に動く | VSCode / `extends` セクション追記済み |
| [VSCode 互換戦略](./npm_package_spec.md#vscode-互換戦略) | ガイド済み | 100 | [npm_usage.md](./npm_usage.md) に移行例 | 利用側での設定変更は各プロジェクト |
| [CI 互換戦略](./npm_package_spec.md#ci-互換戦略) | ガイド済み | 100 | `npm_usage.md` / `examples/` に CI 例 | 利用側 CI 移行は各プロジェクト |
...
仕様 [非推奨化ポリシー - フェーズ1で完了した項目](./npm_package_spec.md#フェーズ1で完了した項目)、[Publishing - フェーズ1優先タスク](./npm_package_spec.md#publishing---フェーズ1優先タスク)、[互換性に関する、移行戦略 - フェーズ1優先タスク](./npm_package_spec.md#互換性に関する移行戦略---フェーズ1優先タスク)、[本リポジトリ `package.json` の `scripts` - フェーズ1優先タスク](./npm_package_spec.md#本リポジトリ-packagejson-の-scripts---フェーズ1優先タスク)、[npm 使い方ガイド との整合 - フェーズ1優先タスク](./npm_package_spec.md#npm-使い方ガイド-との整合---フェーズ1優先タスク) の責務と一致します。
...
* [npm_usage.md](./npm_usage.md) … install / CLI / `package.json` / GitHub Actions / **VSCode・`extends` 移行例**
...
* `package.json` `bin` の互換 CLIを `docs-lint` に統一

### フェーズ1で完了した主な変更 (コード・文書)

| 対象 | 内容 |
| --- | --- |
| `src/bin/run-textlint.ts` | preset 解決、`--profile`、`-h`/`--help`、`-V`/`--version`、textlint を `node` 直実行、NODE_PATH 連結 |
| `package.json` | publish 用 scripts、`files` に root ミラー、`build` にミラー生成、`bin.docs-lint` |
| `tsconfig.build.json` | 本番ビルド用 (`noEmit: false`、`dist/` 出力) |
| `scripts/verify-tarball.cjs` | `presets/*` と `base/` `swift/` `wordpress/` の必須パス・禁止パス検証 |
| `src/scripts/link-preset-layout-compat.ts` | ビルド時に `presets/*` → root `base/` `swift/` `wordpress/` をミラー (npm pack 同梱用) |
| `examples/lint-docs*.yml` | lint ステップを `npx s2j-docs-linter` に更新、npm install 手順コメントを `npm_usage` と整合 |
| `README.md` | 方法2 (npm) を `@s2j/docs-linter` / `s2j-docs-linter` に更新、`npm_usage.md` へ導線 |
| `docsMod/npm_usage.md` | `--profile` / CI / **VSCode・`extends` 移行例** (互換レイアウト) |
| `docsMod/npm_package_spec.md` | Publishing / 互換移行 / npm 使い方ガイド整合 等のフェーズ1優先タスク |

### フェーズ1の残タスク (本リポジトリ)

仕様 [フェーズ1の残タスク](./npm_package_spec.md#フェーズ1の残タスク-本リポジトリ) と一致します。本リポジトリ内の実装タスクは **完了** (実装％ **100%**)。フェーズ1クローズに必要なのは下表の運用タスクのみです。

| # | タスク | 優先度 | 実装％ | 完了条件 |
| ---: | --- | --- | ---: | --- |
| 1 | npmjs への初回 `npm publish --access public` | **必須** (フェーズ1クローズ) | 0 | `npm view @s2j/docs-linter version` が `1.0.10`、`npm install` で取得可能 |
| 2 | 利用側プロジェクトでの受け入れ試験 | 推奨 | 0 | Submodule 併存のまま `npx s2j-docs-linter` と VSCode 互換パスが動作 |
