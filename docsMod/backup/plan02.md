...
先ほど私が懸念したのは、下記のような JavaScript そのものをユーザーがアップロードできる仕組みです。
...
ユーザーごとの差異は、こちらで吸収します。

---

## ESLint の考え方に近い

例えば ESLint も、下記のように、ルールそのものは固定です。ユーザーはパラメータだけ変えます。
...
内部 DB に、「ルール定義」「ユーザー設定」のような構造を持てます。
...
もし将来的に「ユーザーが独自ルールを作りたい」まで考えるなら、JavaScript を書かせるのではなく DSL（設定言語）を作る方が安全です。
