...
* ブラウザ内 textlint 実行は、Chrome 以外では実現できない…と言う「ブラウザ依存」か？
...
しかし `50,000文字` のマニュアルや記事群になると、ユーザー PC で解析することになります。
...
```mermaid
flowchart TD
  A["docs-linter-core"] --> B["Node.js"]
```

理由は、下記だからです。

* WordPress - REST 経由で呼べる
* Spring Boot - REST 経由で呼べる
* Java - REST 経由で呼べる
* 将来のデスクトップアプリ - REST 経由で呼べる
